...
def set_reserva_fields(reserva, form):
    campos_float = [
        ...
        'comision_ejecutivo', 'comision_agencia'  # Agregados los nuevos campos
    ]
    campos_str = [
...
def ranking_ejecutivos():
    ...
    for reserva in reservas_query.all():
        ...
        ranking_data[key]['reserva.usuario.Ganancia_bruta'] += reserva.usuario.Ganancia_bruta

    ranking_final = list(ranking_data.values())
    ranking_final.sort(key=lambda x: x['reserva.usuario.Ganancia_bruta'], reverse=True)
